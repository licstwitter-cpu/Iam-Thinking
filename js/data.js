...
IT.data = (function () {
  ...
  function hash(clave) {
    ...
    return Promise.resolve("fb_" + (h >>> 0).toString(16));
  }
  function claveValida(clave, hashAlmacenado) {
    if (!hashAlmacenado) return Promise.resolve(false);
    return hash(String(clave == null ? "" : clave)).then(function (h) { return h === hashAlmacenado; });
  }
  ...
  /* ---- avatar (con foto opcional) ---- */
  function pintarAvatar(el, p) {
    if (!el || !p) return;
    var bg = p.color || "#5F7BA6";
    if (p.foto) {
      el.className = "avatar avatar-foto";
      ...
      el.className = "avatar avatar-dinamico";
      el.style.background = bg;
  ...
  function crearPerfil(datos, cb) {
    cuandoListo(function () {
      hash(datos.clave || "").then(function (clave) {
        var uid = "u" + pushId();
        var perfil = {
          name: datos.name,
          ...
          foto: datos.foto || "",
          fondoImg: datos.fondoImg || "",
          fondoOscuridad: datos.fondoOscuridad || 0,
          emojis: datos.emojis !== false,
          clave: clave,
          created: Date.now()
        };
  ...
  /* ---- borrado solo para mí (rm = removed for me) ---- */
  function borrarMensaje(chatId, msgId) {
    if (!miUid || !chatId || !msgId) return;
    ref("rm/" + chatId + "/" + miUid + "/" + msgId).set(true);
  }
  function vaciarChat(chatId) {
    if (!miUid || !chatId) return;
    ref("rm/" + chatId + "/" + miUid + "/__all").set(Date.now());
  }
  function observarBorrados(chatId, cb) {
    var r = ref("rm/" + chatId);
    r.on("value", function (s) { cb(s.val() || {}); });
    return r;
  }
  function cerrarBorrados(r) { if (r) r.off("value"); }
  ...
  return {
    ...
    vaciarChat: vaciarChat,
    observarBorrados: observarBorrados,
    cerrarBorrados: cerrarBorrados,
    chatIdDM: chatIdDM,
    leerMeta: leerMeta,
