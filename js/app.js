...
(function () {
  ...
  /* ---------- estilos de fondo (emojis flotantes + paleta) ---------- */
  var ESTILOS = [
    { key: "claro", nombre: "Claro", pack: ["☁️", "☀️", "🌈", "⛅", "🕊️", "✨"] },
    { key: "amor", nombre: "Amor", pack: ["💗", "🌹", "💘", "😍", "💝", "🌸"] },
    { key: "lavanda", nombre: "Lavanda", pack: ["🪻", "💜", "🦋", "🔮", "✨", "👾"] },
    { key: "cielo", nombre: "Cielo", pack: ["☁️", "✈️", "🕊️", "🌤️", "💙", "🪁"] },
    { key: "menta", nombre: "Menta", pack: ["🌿", "🍃", "💚", "🥝", "🌱", "✅"] },
    { key: "sol", nombre: "Sol", pack: ["☀️", "🌻", "🍋", "🌼", "😎", "🥤"] },
    { key: "noche", nombre: "Noche", pack: ["🌙", "⭐", "🌌", "🦉", "🌠", "💫"] },
    { key: "oceano", nombre: "Océano", pack: ["🌊", "🐠", "🐚", "⛵", "🐬", "🦀"] },
    { key: "fuego", nombre: "Fuego", pack: ["🔥", "🧡", "🌋", "⚡", "🦊", "🍂"] },
    { key: "bosque", nombre: "Bosque", pack: ["🌲", "🌳", "🦌", "🍄", "🐿️", "🌱"] },
    { key: "uva", nombre: "Uva", pack: ["🍇", "🍷", "💜", "🍆", "🎆", "🌃"] },
    { key: "cafe", nombre: "Café", pack: ["☕", "🧋", "🍩", "🐻", "🪵", "🌰"] },
    { key: "nieve", nombre: "Nieve", pack: ["❄️", "⛄", "🦢", "🤍", "🧊", "🫧"] },
    { key: "atardecer", nombre: "Atardecer", pack: ["🌇", "🌆", "🌉", "🏙️", "🚗", "🌃"] },
    { key: "playa", nombre: "Playa", pack: ["🏖️", "🌴", "🍹", "🐚", "🌺", "🏄"] },
    { key: "otonio", nombre: "Otoño", pack: ["🍁", "🍂", "🎃", "🌰", "🦃", "🧡"] },
    { key: "cerezo", nombre: "Cerezo", pack: ["🌸", "🌺", "🍡", "🎐", "🦩", "🩷"] },
    { key: "neon", nombre: "Neón", pack: ["🎮", "🤖", "👾", "💿", "🕹️", "⚡"] },
    { key: "oro", nombre: "Oro", pack: ["🏆", "👑", "💛", "⭐", "🥇", "🤩"] },
    { key: "tormenta", nombre: "Tormenta", pack: ["⛈️", "🌩️", "🌧️", "🌪️", "☔", "🌫️"] }
  ];
  var ESTILO_ACTUAL = "claro";

  function estiloDe(key) {
    for (var i = 0; i < ESTILOS.length; i++) {
      if (ESTILOS[i].key === key) return ESTILOS[i];
    }
    return ESTILOS[0];
  }

  function pintarFondo(pack) {
    var f = IT.$("#fondo");
    if (!f) return;
    var total = Math.min(pack.length * 3, 14);
    var html = "";
    for (var i = 0; i < total; i++) {
      var izq = (Math.random() * 100).toFixed(1);
      var tam = (28 + Math.random() * 42).toFixed(0);
      var dur = (16 + Math.random() * 20).toFixed(1);
      var ret = (Math.random() * -34).toFixed(1);
      var va = ((Math.random() * 70) - 35).toFixed(0);
      var op = (0.35 + Math.random() * 0.4).toFixed(2);
      html += "<span class='emoji-fondo' style='left:" + izq + "%;font-size:" + tam + "px;animation-duration:" + dur +
        "s;animation-delay:" + ret + "s;--va:" + va + "px;--op:" + op + "'>" + pack[i % pack.length] + "</span>";
    }
    f.innerHTML = html;
  }

  function aplicarEstilo(key) {
    var e = estiloDe(key);
    ESTILO_ACTUAL = e.key;
    document.documentElement.setAttribute("data-estilo", e.key);
    pintarFondo(e.pack);
  }

  IT.estilos = ESTILOS;
  IT.aplicarEstilo = aplicarEstilo;
  IT.estiloActual = function () { return ESTILO_ACTUAL; };

  var ESTILO_EMOJIS = true;
  function aplicarEmojis(bool) {
    ESTILO_EMOJIS = !!bool;
    var f = IT.$("#fondo");
    if (f) f.style.display = ESTILO_EMOJIS ? "" : "none";
  }
  function aplicarFondoPerfil(p) {
    var manto = IT.$("#fondo-manto");
    var tinta = IT.$("#fondo-tinta");
    if (!manto || !tinta) return;
    if (p && p.fondoImg) {
      manto.className = "fondo-manto on";
      manto.style.backgroundImage = "url('" + p.fondoImg + "')";
      document.documentElement.setAttribute("data-fondo-img", "1");
      tinta.style.opacity = ((p.fondoOscuridad || 0) / 100).toFixed(2);
    } else {
      manto.className = "fondo-manto";
      manto.style.backgroundImage = "";
      document.documentElement.removeAttribute("data-fondo-img");
      tinta.style.opacity = "0";
    }
  }
  IT.aplicarEmojis = aplicarEmojis;
  IT.aplicarFondoPerfil = aplicarFondoPerfil;
  ...
  function entrarUid(uid) {
    D.leerUsuario(uid, function (p) {
      ...
      aplicarEstilo(p.estilo || "claro");
      aplicarFondoPerfil(p);
      aplicarEmojis(p.emojis !== false);
  ...
  function crearPerfil() {
    ...
    D.crearPerfil({ name: nombre, emoji: emojiSel, color: colorSel, clave: clave, estilo: "claro" }, function (uid, perfil) {
      aplicarEstilo(perfil.estilo || "claro");
      aplicarFondoPerfil(perfil);
      aplicarEmojis(true);
