...
IT.media = (function () {
  ...
  var STICKERS = ["😍", "😂", "🥰", "😭", "😎", "🤯", "💖", "🎉", "🙄", "😴",
                  "🤗", "😇", "😘", "💪", "👍", "🥺", "😅", "🔥", "✨", "💔"];
  var LIMITE = { image: 2 * 1024 * 1024, video: 3.5 * 1024 * 1024, audio: 1024 * 1024 };
  ...
  function archivoAUrl(file, maxPx, calidad, cb, err) {
    if (file.size > LIMITE.image) { if (err) return err("Esa foto pesa demasiado (máx 2 MB). Elige otra 🥺"); }
  ...
  function videoAUrl(file, cb, err) {
    if (file.size > LIMITE.video) { if (err) return err("El video es muy grande (máx 3.5 MB). Graba uno más corto 🎬"); }
    var lector = new FileReader();
  ...
  C.recParar = function (cb) {
    ...
    recorder.onstop = function () {
      ...
      chunks = [];
      recorder = null;
      if (blob.size > LIMITE.audio) { C.recError = "El audio supera 1 MB, graba uno más corto 🎙️"; if (cb) cb(null); return; }
      if (cb) cb(blob);
    };
    rec.stop();
  };
